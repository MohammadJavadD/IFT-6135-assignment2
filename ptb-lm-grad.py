...
#          done --model=RNN --optimizer=ADAM --initial_lr=0.0001 --batch_size=20 --seq_len=35 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35 --save_best
#          done --model=GRU --optimizer=SGD_LR_SCHEDULE --initial_lr=10 --batch_size=20 --seq_len=35 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35 --save_best
#          done --model=TRANSFORMER --optimizer=SGD_LR_SCHEDULE --initial_lr=20 --batch_size=128 --seq_len=35 --hidden_size=512 --num_layers=6 --dp_keep_prob=0.9 --save_best
...
#          done --model=RNN --optimizer=SGD --initial_lr=0.0001 --batch_size=20 --seq_len=35 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35
#          done --model=GRU --optimizer=SGD --initial_lr=10 --batch_size=20 --seq_len=35 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35
#          done --model=TRANSFORMER --optimizer=SGD --initial_lr=20 --batch_size=128 --seq_len=35 --hidden_size=512 --num_layers=6 --dp_keep_prob=.9
#          done --model=RNN --optimizer=SGD_LR_SCHEDULE --initial_lr=1 --batch_size=20 --seq_len=35 --hidden_size=512 --num_layers=2 --dp_keep_prob=0.35
#          done --model=GRU --optimizer=ADAM --initial_lr=0.0001 --batch_size=20 --seq_len=35 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35
#          done --model=TRANSFORMER --optimizer=ADAM --initial_lr=0.001 --batch_size=128 --seq_len=35 --hidden_size=512 --num_layers=2 --dp_keep_prob=.9
...
if args.model == 'RNN':
    ...
    save_dir = 'Probleme_4_1/RNN/best_params.pt'
elif args.model == 'GRU':
    model = GRU(emb_size=args.emb_size, hidden_size=args.hidden_size,
                seq_len=args.seq_len, batch_size=args.batch_size,
                vocab_size=vocab_size, num_layers=args.num_layers,
                dp_keep_prob=args.dp_keep_prob)
    save_dir = 'Probleme_4_1/GRU/best_params.pt'
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        grads_norm = np.zeros(seq_len)
        # Loss computation at each time-state
        if is_train == True:
            hidden_t = hidden.clone()
            hidden_t.requires_grad = True
            model.seq_len = 1
            for t in range(seq_len):
                hiddens.append(hidden_t)
                input_t = inputs[t, :].unsqueeze(0)
                outputs_t, hidden_t = model(input_t, hiddens[t])

                if t == seq_len - 1:
                    outputs_t = outputs_t.squeeze()
                    targets_t = targets[t, :]
                    loss_t = loss_fn(outputs_t, targets_t)
                    g = torch.autograd.grad(loss_t, hiddens, retain_graph=True)
                    for i in range(seq_len):
                        grad.append(torch.mean(g[i], dim=1))
                        grads_norm[i] = torch.norm(grad[i], p='fro').cpu().numpy()

                    np.save(args.save_dir + '/grads_RNN', grads_norm)
